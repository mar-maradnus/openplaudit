    ...
    private func stopRecordingAndTranscribe() async {
        ...
        do {
            ...
            if config.summarisation.enabled {
                ...
                let currentResult = result
                if let summary = try? await SyncEngine.applySummarisation(to: currentResult, config: summCfg) {
            ...
            if config.summarisation.enabled {
                ...
                let baseName = recording.wavPath.deletingPathExtension().lastPathComponent
                if let (mmResult, _) = try? await MindMapGenerator.generateAndExport(
                    transcript: formatted, backend: backend,
                    outputDir: dirs.meetingTranscripts, baseName: baseName
                ) {
